...
#
# PROD partial S3 remote-backend configuration. Supplies the concrete values for
...
bucket         = "prod-finance-sp-chain-replacement-tfstate"   # Remote Terraform STATE bucket (ops/state bucket; separate from the pipeline data buckets in prod.tfvars).
key            = "sp-chain-replacement/prod/terraform.tfstate" # Env-specific state object path (key) within the state bucket; note the `prod` path segment.
region         = "us-east-1"                                   # AWS region hosting the state bucket and the state-lock table.
dynamodb_table = "prod-finance-sp-chain-replacement-tflock"    # Terraform STATE-LOCK table. DISTINCT from the S3DynamoDBLogStore coordination table (prod-finance-sp-chain-replacement-logstore, set as dynamodb_table_name in prod.tfvars). Do NOT reuse that value.
encrypt        = true                                          # Enforce server-side encryption (SSE) of the prod state object at rest (the prod state may contain sensitive resource metadata).
