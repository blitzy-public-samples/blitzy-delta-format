...
# -----------------------------------------------------------------------------
# infra/envs/nonprod-backend.hcl
#
# NONPROD partial S3 remote-backend configuration. Supplies the concrete values
# for the empty `backend "s3" {}` block declared in the sibling infra/versions.tf.
#
# Used at init time:
#
#   terraform init -backend-config=envs/nonprod-backend.hcl
#
...
bucket         = "nonprod-finance-sp-chain-replacement-tfstate"   # Remote Terraform STATE bucket (ops/state bucket; separate from the pipeline data buckets in nonprod.tfvars).
key            = "sp-chain-replacement/nonprod/terraform.tfstate" # Env-specific state object path (key) within the state bucket; note the `nonprod` path segment.
region         = "us-east-1"                                      # AWS region hosting the state bucket and the state-lock table.
dynamodb_table = "nonprod-finance-sp-chain-replacement-tflock"    # Terraform STATE-LOCK table. DISTINCT from the S3DynamoDBLogStore coordination table (nonprod-finance-sp-chain-replacement-logstore, set as dynamodb_table_name in nonprod.tfvars). Do NOT reuse that value.
